...
def create_order(request):
    ...
    if request.user.id:  ## Авторизованный пользователь получает инф-ю о корзине
        cart = ProductCartUser(request).cart
       # либо получаем корзину из сессии
    else:
        cart = Cart(request).cart  ## Если пользователь не авторизован, то получаем корзину из сессии
    ...
    if order_form.is_valid():  ## Проверяем валидность
        order_form.save()
        ...
        if request.user.id:  ##  Если пользователь авторизован
            user = User.objects.get(pk=request.user.id)  ## создается переменнная юзер и в нее заносится юзер ай-ди
            order = Order.objects.get(number = order_num)  ## Создается заказ
            order.user = user  ## Добавляет пользователя в заказ
        else:
            # получаем объект самого заказа из БД
            order = Order.objects.get(number = order_num)  ## если пользователя нет, то просто создается заказ
